# -*- coding: utf-8 -*-
"""
Created on Mon Sep 12 14:16:05 2016
...
class tensgrid():
    ...
    __init__(self, pdftype, gridtype, gridshape, limits, N):

    Parameters:
    -----------
    pdftype: list of str [N_vars]
        variable specific type of PDF ("beta", "normal")
    gridtype: list of str [N_vars]
...
class sparsegrid():
    ...
    __init__(self, pdftype, gridtype, gridshape, limits, level, level_max, interaction_order, order_sequence_type, make_grid=True):
    ...
    def __init__(self, pdftype, gridtype, gridshape, limits, level, level_max, interaction_order, order_sequence_type, make_grid=True):
        ...
        # make_grid decide if grid is generated by scratch during initialization or if coords, coords_norm and weights
        # are added manually
        if make_grid:
            ...
            for i_dim in range(self.DIM):
                if self.gridtype[i_dim] == 'fejer2':
                    level_sequence.append(list(range(1,self.level[i_dim]+1)))
                else:
                    level_sequence.append(list(range(self.level[i_dim]+1)))
                if self.order_sequence_type == 'exp':       # order = 2**level + 1
                    if self.gridtype[i_dim] == 'fejer2':    # start with order = 1 @ level = 1
                        order_sequence.append((2**(np.linspace(1, self.level[i_dim], self.level[i_dim]).tolist())-1).tolist())
                        order_sequence[i_dim][0] = 1
                    elif self.gridtype[i_dim] == 'patterson': # start with order = 1 @ level = 0 [1,3,7,15,31,...]
                        order_sequence.append((2**(np.linspace(0, self.level[i_dim], self.level[i_dim]+1).tolist()+1)-1).tolist())
                    else:                                   # start with order = 1 @ level = 0
                        order_sequence.append((2**np.linspace(0, self.level[i_dim], self.level[i_dim]+1)+1).tolist())
                        order_sequence[i_dim][0] = 1
                elif self.order_sequence_type == 'lin':     # order = level
                    if self.gridtype[i_dim] == 'fejer2':    # start with level = 1 @ order = 1
                        order_sequence.append(np.linspace(1, self.level[i_dim]+1, self.level[i_dim]+1).tolist())
                    elif self.gridtype[i_dim] == 'patterson': # start with order = 1 @ level = 0 [1,3,7,15,31,...]
    ...
    #==============================================================================
    #         l_order = misc.cartesian(order_sequence)
    #         l_level = misc.cartesian(level_sequence)
    #
    ...
    #==============================================================================
            ...
            print('Sparsegrid initialized but not generated ... Please add coords / coords_norm and weights manually')
...
class randomgrid():
    def __init__(self, pdftype, gridshape, limits, N, seed=None):
        """
        Generate randomgrid object instance

        __init__(self, pdftype, gridshape, limits, N, seed=None):
