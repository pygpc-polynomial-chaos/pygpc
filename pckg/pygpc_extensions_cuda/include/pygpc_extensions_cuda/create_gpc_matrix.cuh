#ifndef PYGPC_EXTENSIONS_CREATE_GPC_MATRIX_CU_H
#define PYGPC_EXTENSIONS_CREATE_GPC_MATRIX_CU_H
...
__global__ void create_gpc_matrix_cuda_tk(T* ptr_arguments, T* ptr_coeffs,
    T* ptr_result, U n_arguments, U n_dim, U n_basis)
{
    
    if(blockIdx.x * blockDim.x + threadIdx.x < n_arguments) {
        U i_basis = 0;
...
int create_gpc_matrix_cuda_t(T* ptr_arguments, T* ptr_coeffs,
    T* ptr_result, U n_arguments, U n_dim, U n_basis, U n_coeffs)
{
